...
/** Ícone do motorista no mapa: círculo com carro. Tamanho reduzido para não cortar no Android. */
export function DriverMarkerIcon() {
  const isDark = useColorScheme() === 'dark';
  const bg = isDark ? '#FFFFFF' : '#0d0d0d';
  const carFill = isDark ? '#0d0d0d' : '#FFFFFF';

  return (
    <View style={styles.wrap}>
    <View style={[styles.circle, { backgroundColor: bg }]}>
      <Svg width={SIZE} height={SIZE} viewBox="0 0 40 40" fill="none">
        <G clipPath="url(#clip0_driver_car)">
          <Path
