...
type RootNavigationContextValue = { resetToSplash: () => void };
...
export function useRootNavigation(): RootNavigationContextValue {
  ...
  if (!ctx) throw new Error('useRootNavigation must be used inside RootNavigationProvider');
  return ctx;
}
...
export function RootNavigationProvider({
  ...
  const resetToSplash = useCallback(() => {
    navigationRef.current?.dispatch(CommonActions.reset({ index: 0, routes: [{ name: 'Splash' }] }));
  }, [navigationRef]);
