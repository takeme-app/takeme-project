const ERROR_PT: Array<[RegExp, string]> = [
  ...
  [/rate limit|rate_limit|too many requests|429/i, 'Aguarde alguns minutos e tente novamente.'],
  [/session.*expired|refresh token|jwt expired/i, 'Sessão expirada. Faça login novamente.'],
...
export function getUserErrorMessage(error: unknown, fallback: string = DEFAULT_MESSAGE): string {
  ...
  for (const [pattern, message] of ERROR_PT) {
    if (pattern.test(raw) || pattern.test(lower)) return message;
  }
  return fallback;
}
