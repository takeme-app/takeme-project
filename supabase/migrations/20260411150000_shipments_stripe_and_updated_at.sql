-- Cobrança Stripe (charge-shipments) + estorno (process-refund) + auditoria.

ALTER TABLE public.shipments
  ADD COLUMN IF NOT EXISTS stripe_payment_intent_id text NULL,
  ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now();

COMMENT ON COLUMN public.shipments.stripe_payment_intent_id IS 'PaymentIntent Stripe (pi_…) após charge-shipments.';

ALTER TABLE public.dependent_shipments
  ADD COLUMN IF NOT EXISTS stripe_payment_intent_id text NULL,
  ADD COLUMN IF NOT EXISTS updated_at timestamptz NOT NULL DEFAULT now();

COMMENT ON COLUMN public.dependent_shipments.stripe_payment_intent_id IS 'PaymentIntent Stripe após charge-shipments.';
