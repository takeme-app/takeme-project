-- Resolve user id por e-mail normalizado (auth.users + auth.identities), para Edge Functions
-- sem precisar paginar listUsers (evita falhas e custo com muitos usuários).
...
as $$
  select u.id
  from auth.users u
  where lower(trim(both from coalesce(u.email, ''))) = lower(trim(both from coalesce(p_email, '')))
     or exists (
          select 1
...
comment on function public.lookup_auth_user_id_by_normalized_email(text) is
  'Usado pelas Edge Functions (service_role): localiza auth.users.id por e-mail em users.email ou identities.identity_data.email.';
