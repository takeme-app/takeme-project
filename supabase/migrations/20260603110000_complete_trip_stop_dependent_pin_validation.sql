-- =============================================================================
-- Cenário 2 (PDF "Sequência de Solicitação de Código"): Envio de Dependente
-- Restaura validação de PIN no servidor para AMBAS as etapas:
--   - dependent_pickup  (etapa 1-3 do PDF: motorista pede ao responsável-origem)
--   - dependent_dropoff (etapa 5-7 do PDF: motorista pede ao responsável-destino)
--
-- Antes (20260520140000): nenhum dos dois exigia PIN — só o app validava no
-- cliente, criando risco de bypass.
--
-- Depois: ambos exigem 4 dígitos no servidor, comparando com
-- dependent_shipments.pickup_code e .delivery_code respectivamente.
-- Atualiza timestamps e status do envio de dependente.
--
-- Mantém o restante intacto: passageiro pickup com PIN, dropoff sem PIN,
-- encomenda em ambas as pernas com PIN.
-- =============================================================================
...
BEGIN
  ...
  -- ============================================================
  -- DEPENDENTE: PIN obrigatório em embarque E desembarque (PDF cenário 2).
  -- ============================================================
  ...
  -- passenger_dropoff e demais tipos: sem PIN, apenas marcar concluída.
  END IF;
...
COMMENT ON FUNCTION public.complete_trip_stop (uuid, text) IS
  'Conclui parada de viagem combinada validando PIN conforme PDF "Sequência de Solicitação de Código": passenger_pickup, dependent_pickup, dependent_dropoff e shipment/package pickup+dropoff exigem PIN; passenger_dropoff e demais tipos não exigem.';
