-- Valida PIN de embarque/desembarque para paradas `dependent_*` (mesmo padrão de passageiros).
...
BEGIN
  ...
  ELSIF tnorm IN ('dependent_pickup', 'dependent_dropoff') THEN
    ...
    WHERE d.id = v_stop.entity_id
      AND d.scheduled_trip_id = v_trip_id;
...
COMMENT ON FUNCTION public.complete_trip_stop (uuid, text) IS
  'Motorista conclui parada: encomenda (código), passageiro (booking), dependente (dependent_shipments) ou outras sem PIN.';
